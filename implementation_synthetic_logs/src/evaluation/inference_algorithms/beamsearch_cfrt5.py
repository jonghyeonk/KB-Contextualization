...
from src.evaluation.prepare_data import encode_with_group, encode_with_group2, get_predictions, get_pn_fitness
...
def run_experiments(log_data: LogData, compliant_traces: pd.DataFrame, maxlen, predict_size, char_indices,
    ...
    class NodePrediction:
        def __init__(self, crop_trace: pd.DataFrame, probability_of=0):
            ...
            self.cropped_line_time3 = crop_trace[log_data.timestamp_key3].tolist()     #jh
            self.cropped_line_time4 = crop_trace[log_data.timestamp_key4].tolist()     #jh
            
            self.model_input = encode_with_group2(self.cropped_line, self.cropped_line_group,
                                                 self.cropped_line_time, self.cropped_line_time2, #jh
                                                 self.cropped_line_time3, self.cropped_line_time4, #jh
                                                   maxlen, char_indices,
                                                 char_indices_group)
    ...
    def jh(trace, prefix_size, log_data, predict_size, pn_file, target_indices_char, target_char_indices, target_indices_char_group, target_char_indices_group):
        ...
        ground_truth_time3 = trace[log_data.timestamp_key3].tolist()     #jh
        ground_truth_time4 = trace[log_data.timestamp_key4].tolist()     #jh
        ...
        for i in range(predict_size - prefix_size):
            for k in range(current_beam_size):
                ...
                temp_cropped_line_time3 = current_prediction_premis.cropped_line_time3 #jh
                temp_cropped_line_time4 = current_prediction_premis.cropped_line_time4 #jh
                ...
                for j in range(current_beam_size):
                    ...
                    temp_prediction_time3 = temp_cropped_line_time3[j]
                    temp_prediction_time4 = temp_cropped_line_time4[j]
                    ...
                    predicted_row.loc[:, log_data.timestamp_key3] = temp_prediction_time3
                    predicted_row.loc[:, log_data.timestamp_key4] = temp_prediction_time4
        ...
        predicted_time3 = current_prediction_premis.cropped_line_time3[prefix_size:]
        predicted_time4 = current_prediction_premis.cropped_line_time4[prefix_size:]
    ...
    with open(output_file, 'w',encoding='utf-8', newline='') as csvfile:
        ...
        spamwriter.writerow(["Prefix length", "Ground truth", "Predicted", "Damerau-Levenshtein", "Jaccard",
                             ...
                             "Ground truth time", "Predicted time", "Ground truth time2", "Predicted time2",
                             "Ground truth time3", "Predicted time3", "Ground truth time4", "Predicted time4",
    ...
    for prefix_size in range(log_data.evaluation_prefix_start, log_data.evaluation_prefix_end+1):
        ...
        compliant_traces.groupby(log_data.case_name_key).apply(lambda x: jh(x, prefix_size, log_data,
