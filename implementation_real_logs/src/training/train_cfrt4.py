...
def _build_model(max_len, num_features, target_chars, target_chars_group, models_folder):
    ...
    if models_folder == "LSTM":
        ...
        time2_output = Dense(1, activation='ReLU', name='time_output')(processed)        
    ...
    model = Model(main_input, [activity_output, group_output, time2_output, outcome_output])
    model.compile(loss={'act_output': 'categorical_crossentropy', 'group_output': 'categorical_crossentropy',
                        'time_output': 'mean_squared_error', 'outcome_output': 'binary_crossentropy'}, optimizer=opt)
    return model


def _train_model(model, checkpoint_name, x, y_a, y_o, y_g, y_t):
    ...
    history = model.fit(x, {'act_output': y_a, 'outcome_output': y_o, 'group_output': y_g, 'time_output':y_t },
...
def train(log_data: LogData, models_folder: str):
    ...
    training_lines_time = training_time_group4
    ...
    for line, line_group, line_time, outcome in zip(training_lines, training_lines_group, training_lines_time, training_outcomes):
    ...
    for i, sentence in enumerate(sentences):
        ...
        for t, char in enumerate(sentence):
            ...
            x[i, t + leftpad, len(chars) + len(chars_group)+1] = t + 1
    ...
    for fold in range(shared.folds):
        ...
        checkpoint_name = create_checkpoints_path(log_data.log_name.value, models_folder, fold, 'CFRT4')
        _train_model(model, checkpoint_name, x, y_a, y_o, y_g, y_t)
